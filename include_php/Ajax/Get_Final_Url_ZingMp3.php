...
if ($output === false) {
    // Thực hiện code sau khi cURL không thành công
    $command = "node $DuognDanUI_HTML/include_php/Ajax/Youtube_MP3_Link_Play.js $url";
	//$command = "python3 $DuognDanUI_HTML/include_php/Ajax/Youtube_MP3_Link_Play.py " . escapeshellarg($url);
    $connection = ssh2_connect($serverIP, $SSH_Port);
    if (!$connection) {die($Error_HOST);
    if (!ssh2_auth_password($connection, $SSH_TaiKhoan, $SSH_MatKhau)) {die($Error);}
    $stream = ssh2_exec($connection, $command);
    stream_set_blocking($stream, true);
    $stream_out = ssh2_fetch_stream($stream, SSH2_STREAM_STDIO);
    $output = stream_get_contents($stream_out);
